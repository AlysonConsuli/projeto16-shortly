...
import { urlIdSchema } from "../schemas/urlSchema.js"
...
export const usersMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (!user.rows[0]?.id) {
            return res.status(404).send('Não existe esse usuário!')
        }
        if (Number(id) !== userId) {
            return res.status(401).send('Esse id pertence a outro usuário!')
        }
